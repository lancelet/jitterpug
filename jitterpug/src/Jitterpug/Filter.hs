...
instance forall e. Filterable e => Semigroup (PartialFilterResult e) where
    ...
    a <> b = PartialFilterResult weight value
      where
        ...
        value :: e
        value = filterableSum (partialFilterResultValue a)
...
filterSample flt origin sample =
    let fr :: Rect Float
        ...
        dx, dy :: Float
        Geom.V2 dx dy = Geom.subV2 (Sample.samplePos sample) origin
    ...
    in  if Geom.ptInRect fr pn
            then
                let weight :: Float
                    ...
                    value :: e
                    value = filterableScale weight (Sample.sampleValue sample)
                in  PartialFilterResult weight value
            else mempty
{-# INLINABLE filterSample #-}
